...
var PlatformJS = module.exports = function(server) {
	...
	self.clients.addListener('data', function(client, data) {
		self.storage.sendMessage(client, data);
	});
	
	self.clients.addListener('ready', function(client) {
		self.storage.sendMessage(client, "Client '" + client.sid + "' connected");
	});
	
	self.storage = new Storage();
	
	self.storage.addListener('data', function(change) {
		...
		request({uri:self.storage.databaseURL + '/' + change.id}, function(error, response, body) {
			...
			if(response.statusCode == '200') {
				...
				self.clients.broadcast(data.message);
			} 
